...
import { CAREER_HERO } from "../sunita-content";
...
export const CareerHero = () => (
  <section className="min-h-[85vh] relative flex flex-col overflow-hidden">
    ...
    <div className="relative flex-1 flex flex-col justify-center max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 w-full pt-32 sm:pt-36 pb-16">
      ...
      <h1 className="text-[clamp(3rem,8vw,7.5rem)] font-[790] leading-none tracking-[-0.03em] mb-14 sm:mb-16">
        <span className="block overflow-hidden">
          {CAREER_HERO.titleLine1.map((w, i) => (
            <Word key={w} word={w} i={i} />
          ))}
        </span>
        <span className="block overflow-hidden">
          {CAREER_HERO.titleLine2.map((w, i) => (
            <Word
              key={w}
              word={w}
              i={CAREER_HERO.titleLine1.length + i}
              accent={i === CAREER_HERO.titleLine2.length - 1}
            />
          ))}
        </span>
      </h1>
